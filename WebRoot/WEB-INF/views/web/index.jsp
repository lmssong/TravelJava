    ...
    <div class="searchbox">
      ...
      <div class="search">
        <input type="text" class="input1" value="请输入查询关键字"  onFocus="this.value='';" onBlur="if(this.value==''){this.value='请输入查询关键字';}"name="" /><input type="image" src="images/btn.jpg" class="btn" />
      	<a href="http://www.shuangyulin.com" target="_blank" style="color:red;font-size:18px;text-decoration: underline;">点我查看更多设计</a>&nbsp;&nbsp;<span style="color:red;font-size:18px;">联系QQ: 254540457</span>
      </div>
...
<div id="container">
  ...
  <div class="column">
    ...
    <ul class="list">
      <c:forEach var="act" items="${touristspotsList}" varStatus="status" end="4">
		<li class="li1"> <span class="jdpic"><a href="#"><img src="${act.image}" width="224" height="148" /></a></span> <span class="jdtext"><a href="#">${act.name}</a></span>
	        <div class="jdinfo"><span class="zhu">${act.address}</span></div>
	        <div class="jdinfo">预定电话：<span class="price">${act.tel}</span></div>
	      </li>
	</c:forEach>
	<c:if test="${empty  touristspotsList}"><div style="text-align: center;padding-top: 50px;font-size: 14px;">暂无酒店信息</div></c:if>
    </ul>
  </div>
  <!--酒店宾馆-->
  <div class="column">
    ...
    <ul class="list">
    	<c:forEach var="act" items="${hotelList}" varStatus="status" end="4">
    		<li class="li${ status.index + 1}"> <span class="jdpic"><a href="#"><img src="${act.image}" width="224" height="148" /></a></span> <span class="jdtext"><a href="#">${act.name}</a></span>
	        	<div class="jdinfo"><span class="zhu">${act.address}</span></div>
	        	<div class="jdinfo">预定电话：<span class="price">${act.tel}</span></div>
	      	</li>
		</c:forEach>
		<c:if test="${empty  hotelList}"><div style="text-align: center;padding-top: 50px;font-size: 14px;">暂无酒店信息</div></c:if>
    </ul>
  </div>
