...
<div id="container">
  <div class="column">
    ...
    <ul class="list" style="margin-bottom:30px">
      <c:forEach var="act" items="${linesList}" varStatus="status" end="4">
		<li class="li1" style="    width: 1000px;margin-left: 10px;padding: 15px;height: auto;border-radius: 6px;box-shadow: 0px 0px 11px 5px #dcd3c0;"> 
			<div class="jdinfo" style="width: 1000px;height: 30px;font-size: 18px;font-weight: 600;"><h3>线路名称：${act.name}</h3></div>
	        <div class="jdinfo" style="width:1000px;color:red">
	        	起点位置：${act.beginAddress}
	        	-
	        	终点位置：${act.endAddress}
	        </div>
	        <div class="jdinfo" style="width:1000px">
	        	<span>线路说明：${act.introduce}</span><a href="w/lineDetail/${act.id}"><span class="price">详情</span></a></div>
	      </li>
	</c:forEach>
	<c:if test="${empty  linesList}"><div style="text-align: center;padding-top: 50px;font-size: 14px;">暂无线路信息</div></c:if>
    </ul>
  </div>
